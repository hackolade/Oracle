...
class AbstractDualityViewFeDdlCreator {
	 ...
	 * @throws {Error}
	 * */
	constructor(ddlTemplates, assignTemplates, prepareName, getNamePrefixedWithSchemaName) {
	...
	static isRegularDualityViewField(element = {}) {
		return (
			AbstractDualityViewFeDdlCreator.isRegularDualityViewFieldOnRootLevelOrInObjectSubquery(element) ||
	...
	static getPathToReferencedColumn(propertyJsonSchema, parent) {
		...
		if (
			AbstractDualityViewFeDdlCreator.isRegularDualityViewFieldOnRootLevelOrInObjectSubquery(propertyJsonSchema)
		) {
			...
			pathToReferencedColumn = [...(parent.joinedCollectionRefIdPath || []), propertyJsonSchema.ref];
		}
