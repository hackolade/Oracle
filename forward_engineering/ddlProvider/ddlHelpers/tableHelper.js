...
module.exports = ({
	...
	const getBlockChainClause =
		...
		({
			...
			const hasBlockchainPropertiesInTextProperties =
				_.includes(textTableProperties, blockchain_table_retention_clause) ||
	...
	const getExternalTableClause = value => {
		...
		const locationList = `(${_.map(
			location,
			({ location_directory, location_specifier }) =>
				`${location_directory ? `${location_directory}:` : ''}` + `'${location_specifier}'`,
		).join(', ')})`;
		const accessParamType =
			access_parameter_type === 'NONE'
				? ''
				: `ACCESS PARAMETERS ${clob_subquery ? `USING CLOB ${clob_subquery}` : opaque_format_spec}`;
		const externalData =
			`${default_directory && !_.isEmpty(default_directory) ? ` DEFAULT DIRECTORY ${default_directory}` : ''}` +
			accessParamType +
			`${location && !_.isEmpty(location) ? ` LOCATION ${locationList}` : ''}`;
		return (
			` (TYPE ${_.toUpper(access_driver_type)} ${externalData})` + ` REJECT LIMIT ${reject_limit || 'UNLIMITED'} `
		);
	};
	...
	const getPartitionClause = (value, isActivated) => {
		switch (value.partitionBy) {
			...
			case 'composite list': {
				const subpartition = getSubpartition(value, isActivated);
				return (
					getTablespaceList(value.store_in_tablespaces, 'AUTOMATIC') +
					subpartition +
	...
	const getSubpartition = (
		{ subpartitionType, subpartitionDescription, compositePartitionKey },
		isParentActivated,
	) => {
		if (subpartitionType) {
	...
	const getTablespaceList = (store_in_tablespaces, word, key = 'store_in_tablespace') =>
		!_.isEmpty(store_in_tablespaces)
			? `${word ? ` ${word}` : ''} STORE IN (${_.map(store_in_tablespaces, val => val[key]).join(', ')})`
			: '';
