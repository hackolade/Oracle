  ...
  return {
    ...
    createSchema({ schemaName, ifNotExist, dbVersion, sequences, isActivated = true }) {
      ...
      if (!usingTryCatchWrapper) {
        return commentIfDeactivated(
          schemaStatement + statementTerminator + emptyLineSeparator + alterSessionStatement + schemaSequencesStatement,
          { isActivated },
        );
      }
    ...
    createForeignKeyConstraint(
      ...
      const foreignKeyStatement = assignTemplates(templates.createForeignKeyConstraint, {
        primaryTable: getNamePrefixedWithSchemaName(primaryTable, primarySchemaName),
        name: name ? `CONSTRAINT ${prepareName(name)}` : '',
    ...
    createTable(
      ...
      const dividedNotNullConstraints = divideIntoActivatedAndDeactivated(notNullConstraints, key => key.statement);
